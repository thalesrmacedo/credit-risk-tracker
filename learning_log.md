...
* A documentação descreve quatro categorias para EDUCATION e três para MARRIAGE, porém o dataset contém valores adicionais. O significado desses códigos não está explicitamente descrito na documentação consultada e deverá ser investigado antes do tratamento dos dados. As categorias adicionais apresentam baixa representatividade, especialmente em MARRIAGE, mas não serão removidas nesta etapa sem uma justificativa sobre seu significado e impacto. Para tal análise, foram usados os comandos df.value_counts e pd.crosstab.
*As variáveis PAY_0, PAY_2, PAY_3, PAY_4, PAY_5 e PAY_6 apresentaram valores entre -2 e 8. A documentação descreve os valores relacionados ao status de pagamento, porém existem códigos que precisam ser interpretados com cuidado, como -2 e 0, que não foram adequadamente descritos na documentação consultada. Esses dois códigos apresentam alta representatividade nas variáveis de pagamento e, por isso, não serão descartados nesta etapa. O tratamento definitivo será definido posteriormente. Para tal análise, foram usados os comandos df.value_counts e pd.crosstab.
...
* Como o dataset original não apresenta valores ausentes, foram introduzidos artificialmente dois valores NaN em uma cópia do dataset com finalidade exclusivamente didática, permitindo demonstrar o processo de identificação, análise e tratamento de valores ausentes.
